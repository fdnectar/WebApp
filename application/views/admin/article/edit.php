                ...
                <div class="card card-primary">
                    <div class="card-header">
                        <div class="card-title">
                            <h5>Edit Article <?php echo $article['title'] ?> </h5>
                        </div>
                    </div>
                    <form action="<?php echo base_url() . 'admin/article/edit/' . $article['id']; ?>" name="categoryForm" id="categoryForm" method="post" enctype="multipart/form-data">
                        <div class="card-body">
                            <div class="form-group">
                                <label>Category error <?php echo validation_errors(); ?></label>
                            ...
                            <div class="form-group">
                                <label>Tilte</label>
                                <input type="text" class="form-control <?php echo (form_error('title') != "") ? 'is-invalid' : ''; ?>" name="title" id="title" value="<?php echo set_value('title', $article['title']); ?>">
                            ...
                            <div class="form-group">
                                ...
                                <input type="file" name="image" id="image" class="<?php echo (!empty('imageError')) ? 'is-invalid' : ''; ?>"><br>
                                <?php $path = './assets/images/articles/thumb_admin/' . $article['image'];
                                if ($article['image'] != "" && file_exists($path)) { ?>
                                    <img width="250" class="mt-3" src="<?php echo base_url('assets/images/articles/thumb_admin/' . $article['image']); ?>" alt="">
                                <?php  } ?>
                            ...
                            <div class="custom-control custom-radio float-left">
                                <input type="radio" id="statusActive" name="status" checked="" class="custom-control-input" value="1" <?php echo ($article['status'] == 1) ? 'checked' : ''; ?>>
                                <label for="statusActive" class="custom-control-label">Active</label>
                            </div>
                            <div class="custom-control custom-radio float-left ml-3">
                                <input type="radio" id="statusInactive" name="status" class="custom-control-input" value="0" <?php echo ($article['status'] == 0) ? 'checked' : ''; ?>>
