        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_FILES['image']['name'])) {
                ...
                if ($this->upload->do_upload('image')) {
                    ...
                    //unlink images
                    $path = './assets/images/articles/thumb_admin/' . $article['image'];
                    if ($article['image'] != "" && file_exists($path)) {
                        unlink($path);
                    }

                    $path = './assets/images/articles/thumb_front/' . $article['image'];
                    if ($article['image'] != "" && file_exists($path)) {
                        unlink($path);
                    }

                    $path = './assets/images/articles/' . $article['image'];
                    if ($article['image'] != "" && file_exists($path)) {
                        unlink($path);
                    }

                    //resizing images and storing into the following path
                    resizeImage($config['upload_path'] . $data['file_name'], $config['upload_path'] . 'thumb_front/' . $data['file_name'], 1120, 800);
                    resizeImage($config['upload_path'] . $data['file_name'], $config['upload_path'] . 'thumb_admin/' . $data['file_name'], 300, 250);
